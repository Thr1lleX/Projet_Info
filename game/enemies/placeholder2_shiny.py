...
class Placeholder2(Enemy):
    ...
    def __init__(self, scale, x, y):
        ...
        sprite = QPixmap("assets/placeholder2_shiny.png").scaled(
    ...
    def die(self):
        ...
        if random.randint(0, 10) == 0:
            self.rick()
        # flags composites selon ce qui est déjà mort / déjà parlé
        if scene:
            if scene.get_flag("tung_vaincu"):
            ...
            if scene.get_flag("mayor_spoke_first"):
                scene.session_flags["shiny_dead_spoke"] = True
        super().die()
        if scene and hasattr(scene, "dialogue_manager"):
            scene.dialogue_manager.start("shiny_mort")
